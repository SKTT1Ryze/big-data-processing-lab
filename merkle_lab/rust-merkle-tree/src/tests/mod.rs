...
use crate::merkle_tree::{MerkleTree, calculate_height};
use crate::hash::*;
...
fn test_get_element() {
    ...
    assert_eq!(2, *db.get_idx(1).unwrap());
    assert_eq!(6664, *db.get_idx(4).unwrap());
}
...
fn test_hash() {
    assert_eq!("2cf24dba5fb0a30e26e83b2ac5b9e29e1b161e5c1fa7425e73043362938b9824",
               hash_leaf(&"hello".to_string()));
}

#[test]
fn test_combined_hash() {
    let hello_hash = hash_leaf(&"hello".to_string());
    let world_hash = hash_leaf(&"world".to_string());
    let node_hash = hash_mid(&hello_hash, &world_hash);
...
fn test_get_values() {
    ...
    assert_eq!(db.get_vals(), Some(vec![0, 1, 2, 3, 4]));
}
...
fn test_root_calculation() {
    ...
    let h3 = hash_leaf(&3);
    let h4 = hash_leaf(&4);
    let h12 = hash_mid(&h1, &h2);
    let h34 = hash_mid(&h3, &h4);
    let root = hash_mid(&h12, &h34);
